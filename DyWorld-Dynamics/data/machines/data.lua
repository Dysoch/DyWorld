
-- Tier 1 --
require(DyDs_data_machines.. "tier-1.kiln-basic")
require(DyDs_data_machines.. "tier-1.storage-small")
require(DyDs_data_machines.. "tier-1.basic-burner-drill")
require(DyDs_data_machines.. "tier-1.basic-drill")
require(DyDs_data_machines.. "tier-1.basic-treefarm")
require(DyDs_data_machines.. "tier-1.burner-assembler")
require(DyDs_data_machines.. "tier-1.burner-radar")
require(DyDs_data_machines.. "tier-1.assembler")
require(DyDs_data_machines.. "tier-1.lab-1")
require(DyDs_data_machines.. "tier-1.basic-boiler")
require(DyDs_data_machines.. "tier-1.basic-steam-engine")
require(DyDs_data_machines.. "tier-1.basic-power-pole")
require(DyDs_data_machines.. "tier-1.heal-station")
require(DyDs_data_machines.. "tier-1.grinder")
require(DyDs_data_machines.. "tier-1.recycler")
require(DyDs_data_machines.. "tier-1.single-recipe-machines")

-- Tier 2 --
require(DyDs_data_machines.. "tier-2.air-filter")
require(DyDs_data_machines.. "tier-2.bloomery")
require(DyDs_data_machines.. "tier-2.caster")
require(DyDs_data_machines.. "tier-2.blast-furnace")
require(DyDs_data_machines.. "tier-2.gas-engine")
require(DyDs_data_machines.. "tier-2.gas-extractor")
require(DyDs_data_machines.. "tier-2.oil-extractor")
require(DyDs_data_machines.. "tier-2.oil-heater")
require(DyDs_data_machines.. "tier-2.rocket-silo")
require(DyDs_data_machines.. "tier-2.radar")
require(DyDs_data_machines.. "tier-2.lab-2")
require(DyDs_data_machines.. "tier-2.power-pole")
require(DyDs_data_machines.. "tier-2.single-recipe-machines")

-- Tier 3 --
require(DyDs_data_machines.. "tier-3.rocket-silo")
require(DyDs_data_machines.. "tier-3.assembler")
require(DyDs_data_machines.. "tier-3.radar")
require(DyDs_data_machines.. "tier-3.greenhouse")
require(DyDs_data_machines.. "tier-3.lab-3")
require(DyDs_data_machines.. "tier-3.lab-4")

-- Tier 4 --
require(DyDs_data_machines.. "tier-4.rocket-silo")
require(DyDs_data_machines.. "tier-4.radar")
require(DyDs_data_machines.. "tier-4.assembler")

-- Tier 5 --
require(DyDs_data_machines.. "tier-5.rocket-silo")
require(DyDs_data_machines.. "tier-5.radar")

