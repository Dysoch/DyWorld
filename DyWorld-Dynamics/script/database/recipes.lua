


-- this data has moved to DyWorld-Dynamics/script/database/story.lua

Story_Recipes  = {
	----- Act 1, Phase 1 -----
	-- ["charcoal-basic"] = {act = 1, phase = 1},
	-- ["stone-brick-basic-1"] = {act = 1, phase = 1},
	-- ["stone-brick-basic-2"] = {act = 1, phase = 1},
	-- ["stone-brick-basic-3"] = {act = 1, phase = 1},
	-- ["stone-brick-basic-4"] = {act = 1, phase = 1},
	-- ["basic-kiln"] = {act = 1, phase = 1},
	-- ["small-storage"] = {act = 1, phase = 1},
	
	----- Act 1, Phase 2 -----
	-- ["basic-med-pack"] = {act = 1, phase = 2},
	-- ["9mm-bullet-bronze"] = {act = 1, phase = 2},
	-- ["9mm-mag-bronze"] = {act = 1, phase = 2},
	-- ["9mm-mag-bronze-del"] = {act = 1, phase = 2},
	-- ["9mm-pistol"] = {act = 1, phase = 2},
	-- ["basic-burner-drill"] = {act = 1, phase = 2},
	
	----- Act 1, Phase 3 -----
	-- ["bronze-basic"] = {act = 1, phase = 3},
	-- ["stone-brick-wall"] = {act = 1, phase = 3},
	-- ["basic-9mm-turret"] = {act = 1, phase = 3},
	-- ["basic-armor"] = {act = 1, phase = 3},
	
	----- Act 1, Phase 4 -----
	-- ["basic-treefarm"] = {act = 1, phase = 4},
	-- ["rain-collecting"] = {act = 1, phase = 4},
	-- ["rain-collector"] = {act = 1, phase = 4},
	-- ["offshore-pump"] = {act = 1, phase = 4},
	-- ["burner-turbine"] = {act = 1, phase = 4}, 
	-- ["basic-power-pole"] = {act = 1, phase = 4},
	-- ["bronze-pipe"] = {act = 1, phase = 4},
	-- ["bronze-pipe-to-ground"] = {act = 1, phase = 4},
	-- ["basic-repair-pack"] = {act = 1, phase = 4},
	
	----- Act 1, Phase 5 -----
	-- ["transport-belt-1"] = {act = 1, phase = 5},
	-- ["underground-belt-1"] = {act = 1, phase = 5},
	-- ["splitter-1"] = {act = 1, phase = 5},
	-- ["burner-inserter"] = {act = 1, phase = 5},
	-- ["bronze-gear"] = {act = 1, phase = 5},
	
	----- Act 1, Phase 6 -----
	-- ["burner-assembler"] = {act = 1, phase = 6},
	-- ["loader-1"] = {act = 1, phase = 6},
	-- ["control-board-1"] = {act = 1, phase = 6},
	-- ["stone-brick-gate"] = {act = 1, phase = 6},
	-- ["tin-basic"] = {act = 1, phase = 6},
	-- ["tin-basic-2"] = {act = 1, phase = 6},
	-- ["9mm-bullet-tin"] = {act = 1, phase = 6},
	-- ["9mm-mag-tin"] = {act = 1, phase = 6},
	-- ["9mm-mag-tin-del"] = {act = 1, phase = 6},
	
	----- Act 1, Phase 7 -----
	-- ["green-wire"] = {act = 1, phase = 7},
	-- ["red-wire"] = {act = 1, phase = 7},
	-- ["med-pack"] = {act = 1, phase = 7},
	
	----- Act 1, Phase 8 -----
	-- ["cast-iron-1"] = {act = 1, phase = 8},
	-- ["cast-iron-1b"] = {act = 1, phase = 8},
	-- ["cast-copper-1"] = {act = 1, phase = 8},
	-- ["cast-copper-1b"] = {act = 1, phase = 8},
	-- ["small-storage-2"] = {act = 1, phase = 8},
	-- ["iron-gear"] = {act = 1, phase = 8},
	-- ["copper-cable"] = {act = 1, phase = 8},
	-- ["9mm-bullet-copper"] = {act = 1, phase = 8},
	-- ["9mm-mag-copper"] = {act = 1, phase = 8},
	-- ["9mm-mag-copper-del"] = {act = 1, phase = 8},
	
	----- Act 1, Phase 9 -----
	-- ["basic-power-armor"] = {act = 1, phase = 9},
	-- ["ammo-nano-constructors"] = {act = 1, phase = 9},
	-- ["gun-nano-emitter"] = {act = 1, phase = 9},
	
	----- Act 1, Phase 10 -----
	-- ["basic-assembler"] = {act = 1, phase = 10},
	-- ["burner-radar"] = {act = 1, phase = 10}, 
	-- ["lab-1"] = {act = 1, phase = 10},
	-- ["basic-electric-drill"] = {act = 1, phase = 10},
	-- ["basic-boiler"] = {act = 1, phase = 10},
	-- ["basic-steam-engine"] = {act = 1, phase = 10},
	-- ["solar-panel-equipment-1"] = {act = 1, phase = 10},
	-- ["belt-immunity-equipment"] = {act = 1, phase = 10},
	-- ["9mm-bullet-iron"] = {act = 1, phase = 10},
	-- ["9mm-mag-iron"] = {act = 1, phase = 10},
	-- ["9mm-mag-iron-del"] = {act = 1, phase = 10},
	-- ["car-1"] = {act = 1, phase = 10},
	-- ["boat-1"] = {act = 1, phase = 10},
	
	----- Act 2, Phase 3 -----
	-- ["lab-2"] = {act = 2, phase = 3},
	-- ["lab-4"] = {act = 2, phase = 3},
	-- ["food-implant"] = {act = 2, phase = 3},
	-- ["small-storage-3"] = {act = 2, phase = 3},
	
	----- Act 2, Phase 4 -----
	-- ["health-implant"] = {act = 2, phase = 4},
	
	----- Act 2, Phase 9 -----
	-- ["asteroid-miner-titanium"] = {act = 2, phase = 9},
	-- ["lab-3"] = {act = 2, phase = 9},
	
	----- Act 3, Phase 1 -----
	
}
