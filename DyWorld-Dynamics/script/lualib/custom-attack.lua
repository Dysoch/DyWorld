...
function DyWorld_Base_Attack(Surface)
	...
	if ((global.dyworld.game_stats.wave_spawners and global.dyworld.game_stats.wave_spawners[Surface]) and (global.dyworld.game_stats.wave_spawners[Surface].spawners_amount and global.dyworld.game_stats.wave_spawners[Surface].spawners_amount >= 1) and (Str and Str >= 1)) then
		local Spawned = 0
		for i = 1, Str do
		...
		DyWorld_Custom_Difficulty_Change(-1 * math.random(((Spawned * 5000) / 2), (Spawned * 5000)))
	end
end


function DyWorld_Custom_Difficulty_Change(Amount)
